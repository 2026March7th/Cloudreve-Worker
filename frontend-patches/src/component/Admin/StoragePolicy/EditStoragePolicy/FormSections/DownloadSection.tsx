...
const DownloadSection = () => {
  ...
  // 边缘版专属：把中转下载与缩略图内容缓存到 Cloudflare 边缘 CDN（Cache API）
  const onEdgeCacheChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setPolicy((p: StoragePolicy) => ({
        ...p,
        settings: { ...p.settings, edge_cache: e.target.checked ? true : undefined },
      }));
    },
    [setPolicy],
  );
      ...
      <SettingSectionContent>
        ...
        {values.type !== PolicyType.local && (
          <SettingForm lgWidth={5}>
            <FormControl fullWidth>
              <FormControlLabel
                control={<Switch checked={values.settings?.edge_cache ?? false} onChange={onEdgeCacheChange} />}
                label="下载内容缓存到 Cloudflare 边缘 CDN"
              />
              <NoMarginHelperText>
                开启后，中转下载与缩略图会缓存在 Cloudflare 边缘节点：同文件重复下载直接边缘命中，
                不再回源存储，速度显著更快；文件更新后自动失效。直链模式（302 到存储源站）不受影响。
              </NoMarginHelperText>
            </FormControl>
          </SettingForm>
        )}
