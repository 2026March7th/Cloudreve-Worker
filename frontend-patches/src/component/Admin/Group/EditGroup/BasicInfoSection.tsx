// [edge patch] 组基本信息区块。
// 改动：① 存储策略选择改多选（edges.storage_policies 数组，对齐后端
// group_storage_policies 多对多）；② 移除「Pro 可多选」提示行 —— 多选在
// edge 版是自建的正式能力，不再是 Pro 卖点。
  ...
  const onPolicyChange = useCallback(
    (value: number[]) => {
      setGroup((p: GroupEnt) => ({
        ...
        edges: {
          ...p.edges,
          storage_policies: value.map((id) => ({ id })) as never,
        },
      }));
    },
    [setGroup],
          ...
          <>
            <SettingForm title={t("group.availablePolicies")} lgWidth={5}>
              <PolicySelectionInput
                value={((values.edges?.storage_policies ?? []) as { id: number }[]).map((p) => p.id)}
