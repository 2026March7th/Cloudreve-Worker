...
const FileForm = () => {
  ...
  // [edge patch] 文件存储策略：原版此处是写死 onChange={() => {}} 的 Pro 装饰位。
  // 改为真实可编辑，提交时由 FileDialog 把整个 modifiedValues 交给 PUT /admin/file/:id。
  const onStoragePolicyChange = useCallback(
    (policyId: number) => {
      setFile((prev) => ({ ...prev, storage_policy_files: policyId }));
    },
    [setFile],
  );
