...
import { sendPurchaseShare } from "../../../api/api.ts";
...
const BreadcrumbButton = ({ name, is_root, is_latest, path, displayOnly, ...rest }: BreadcrumbButtonProps) => {
  ...
  const paidScore = shareInfo?.score ?? 0;
  const isPaidShare = !!shareInfo && paidScore > 0;
  const hasPurchased = localPurchased || shareInfo?.purchased;

  const handlePurchase = async () => {
    if (!shareInfo || hasPurchased || buying) return;
    setBuying(true);
    try {
      await dispatch(sendPurchaseShare(shareInfo.id));
      setLocalPurchased(true);
      enqueueSnackbar({
        message: "购买成功，现在可以下载该分享的内容了。",
        variant: "success",
        action: DefaultCloseAction,
      });
    } catch (e: any) {
      enqueueSnackbar({
        message: (e && e.message) || "购买失败，请稍后重试。",
        variant: "error",
        action: DefaultCloseAction,
      });
    } finally {
  ...
  return (
    <>
      ...
      {isPaidShare && (
        <Button
          size="small"
          variant={hasPurchased ? "outlined" : "contained"}
          color={hasPurchased ? "success" : "primary"}
